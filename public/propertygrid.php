...
"root", "");
$sort = isset($_GET['sort']) && $_GET['sort'] ? trim($_GET['sort']) : '';
$order = '';
if($sort == 'area') {
    $order = ' ORDER BY area DESC';
} elseif($sort == 'price-low'){
    $order = ' ORDER BY price ASC';
} elseif($sort == 'price-height'){
    $order = ' ORDER BY price DESC';
}
$locationGet = isset($_GET['location']) && $_GET['location'] ? trim($_GET['location']) : '';
$typeGet = isset($_GET['type']) && $_GET['type'] ? trim($_GET['type']) : '';
$priceGet = isset($_GET['price']) && $_GET['price'] ? trim($_GET['price']) : '';
if($priceGet == "<50K"){
    $priceQuery = "price < 50000";
} else if($priceGet == "50K-100K"){
    $priceQuery = "price BETWEEN 50000 AND 100000";
} else if($priceGet == ">100K"){
    $priceQuery = "price > 100000";
}
if(isset($priceQuery) && isset($locationGet) && isset($typeGet)){
    $where = "WHERE location = '{$locationGet}' AND type ='{$typeGet}' AND {$priceQuery} ";   
} else {
    $where = "";
}
$sqlQueryHouse = "SELECT * FROM house {$order} {$where} LIMIT 8";
$stmtHouse = $connect->prepare($sqlQueryHouse);
$stmtHouse->execute();
$dataHouse = $stmtHouse->fetchAll();
                            ...
                            <div class="search-property d-flex flex-wrap">
                                ...
                                <form action="propertylist.php" method="post">
                                    <div class="search-form rounded-pill">
                                        <input class="rounded-pill" type="text" placeholder="Search your keyword" name="keyword">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                      ...
                                      <li><a class="dropdown-item <?= $sort == 'price-height' ? 'active' : ''; ?>" href="?sort=price-height">Price (Max -> Min)</a></li>
                                    </ul>
                                </div>
                                <i class="fa-solid fa-bars"></i>
                            </div>
                        </div>
                        <?php foreach($dataHouse as $keyHouse => $house):?>
                        <div class="col-lg-6">
                            
                            <div class="property-item">
                                <a href="./propertydetail.php?id=<?= $house['id']?>">
                                    <div class="property-img" style="background-image: url('<?= $house['image']?>')">
                                ...
                                <div class="property-info">
                                    <a href="./propertydetail.php?id=<?= $house['id']?>">    
                                        <h3><?= $house['location'] ." " . $house['type']?></h3>
                                    </a>
